...
def market_context_from_dfs(spy_df: pd.DataFrame, qqq_df: pd.DataFrame, vix_df: pd.DataFrame) -> Dict[str, float | bool]:
    ...
    # "Near 52W high" = within 3% of the rolling 252-day high
    ...
    return {
        ...
        "spy_near_52w_high": spy_near_52w_high,
    }
...
def is_eligible(stock_row: pd.Series, regime: str = "BULL") -> bool:
    ...
    ADX floor is regime-dynamic: in strong/momentum markets ADX naturally
    compresses after consolidation — a static floor would miss valid entries.
    """
    try:
...
def is_csp_eligible(stock_row: pd.Series, *, allow_below_200: bool = False,
    ...
    # Price ceiling — 0.0 means disabled (no cap)
    ...
    if min_slope is None:
        # MOMENTUM/STRONG_BULL: price-relative check instead of slope.
...
def ema8_pullback_signal(stock_row: pd.Series, regime: str = "BULL") -> bool:
    """EMA8 pullback entry signal — primary signal for trending/momentum markets.

    Entry conditions (ALL must be true):
      1. Price structure: EMA8 > EMA21 > SMA50 (stacked uptrend)
      2. Price is touching or just below EMA8 (within STOCK_EMA8_BAND %)
      3. RSI(14) is in the healthy pullback zone (not overbought, not wrecked)
      4. ADX > floor (trend has real momentum, not just drift)

    Classic "buy the dip in an uptrend" — works best in MOMENTUM and STRONG_BULL.
    """
    try:
        close  = float(stock_row["Close"])
    ...
    # 1. Stacked uptrend structure
    if not (ema8 > ema21 > sma50):
        return False

    # 2. Price near EMA8 — allow slightly below (dip through) or just above (riding it)
    band = float(regime_val(STOCK_EMA8_BAND, regime, 0.018))
    pct_from_ema8 = (close - ema8) / max(ema8, 1e-9)
    if not (-band * 1.5 <= pct_from_ema8 <= band):
        return False

    # 3. RSI(14) in healthy pullback zone
...
def pullback_signal(stock_row: pd.Series, regime: str = "BULL") -> bool:
    ...
    try:
        ...
        return False
    rsi_max  = float(regime_val(STOCK_PULLBACK_RSI2_MAX, regime, 8.0))
    ema_band = float(regime_val(STOCK_PULLBACK_EMA_BAND, regime, 0.025))
    rsi_ok   = rsi2 < rsi_max
...
def breakout_signal(stock_row: pd.Series, regime: str = "BULL") -> bool:
    ...
    try:
        ...
        return False
    vol_mult = float(regime_val(STOCK_BREAKOUT_VOL_MULT, regime, 1.2))
    return bool(close > high20 and vol > vol_mult * vol_sma)


def nextday_valid_for_entry(signal: str, last: pd.Series) -> bool:
    """Check if a signal is still valid/actionable for next-day entry.

    Only blocks blow-off tops on breakouts (price extended > 1 ATR above
    the breakout level). EMA8 and RSI(2) pullbacks are always valid next
    day — they are buy-the-dip setups, not chases.

    FIXED: removed the redundant standalone volume check that was silently
    killing valid breakout signals which already passed breakout_signal().
    """
    ...
    # Only block blow-off tops — price already extended > 1 ATR above breakout level
    if signal == "BREAKOUT" and atr > 0 and close > high20 + atr:
        return False
...
def plan_stock_trade(
    *,
    ...
    """Build a paper trade plan with account-proportional sizing.

    Sizing philosophy:
      - Max position = INDIVIDUAL_STOCK_CAP * STOCK_MAX_POSITION_PCT[regime]
      - Stop is ATR-based (tighter for EMA8 entries, standard for others)
      - No fixed risk % — you see the position size and stop, you decide
      - Remaining capital check prevents exceeding the stock slice
    """
    ...
    # Stop distance depends on signal type
    if signal == "EMA8_PULLBACK":
        # Tighter stop: just below EMA8 minus ATR buffer
    ...
    # ── Account-proportional sizing ───────────────────────────────────────────
    # No fixed risk % — max position is a regime-dynamic fraction of the stock slice.
    # You see the full position size, stop, and risk on every trade; you decide.
    ...
    return {
        ...
        "notes": (
            f"{signal} | max_pos=${max_pos_value:,.0f} ({max_pos_pct*100:.0f}% of ${acct_size:,.0f}) "
...
def execute_stock_plan(today: dt.date, plan: dict) -> str:
    """Paper 'execution': record OPEN stock position (filled at close).

    IRA/ROTH  → retirement_positions.csv (buy-and-hold)
    INDIVIDUAL → stock_positions.csv (swing trades)
    """
...
def update_and_close_stock_positions(today: dt.date, mkt: Dict[str, float | bool]) -> Dict[str, List[str]]:
    """Update OPEN stock positions and close them if stop/target hit."""
    ensure_stock_files()
    rows = load_stock_positions()
    if not rows:
...
def csp_already_logged(
    ...
    for r in ledger_rows:
        try:
            ...
            log.debug("csp_already_logged: bad row data: %s", e)
            continue
    return False


def has_upcoming_ex_dividend(ticker: str, days_window: int = 10) -> bool:
    """Return True if the stock has an ex-dividend date within days_window days."""
    ...
    try:
        ...
        for ts in divs.index:
            ...
            if today <= ex_date <= cutoff:
                log.info("has_upcoming_ex_dividend: %s ex-div %s within %d days — skipping CSP open",
...
def evaluate_csp_candidate(
    ...
    *,
    ...
    try:
        ...
        raw_strike = _suggest_put_strike(stock_last, atr_mult=atr_mult, risk_off=risk_off,
        ...
        if contracts < 1:
            log.debug("evaluate_csp_candidate: %s skipped — 1 contract ($%.0f) > per-trade budget ($%.0f)",
        ...
        return {
            ...
            "fill_premium": float(fill_premium),
    ...
    *,
    ...
    for idea in enriched:
        ...
        if sector != "OTHER" and sector_counts.get(sector, 0) >= sector_cap:
            log.debug("CSP %s skipped: sector %s already at limit (%d) for regime %s",
    ...
    if tkr:
        for r in rows:
            ...
            if (r.get("ticker") or "").strip().upper() == tkr:
                ...
                return (existing_id or make_csp_position_id(tkr, r.get("expiry") or "",
...
def process_csp_take_profits(today: dt.date, regime: str = "BULL") -> Dict[str, List[str]]:
    """Close OPEN CSPs that have decayed to the take-profit threshold."""
    ensure_positions_files()
    rows = load_csv_rows(CSP_POSITIONS_FILE)
    if not rows:
        return {"closed": []}
    ...
    for r in rows:
        ...
        try:
            ...
            if bid <= 0 or ask <= 0 or ask < bid:
                log.info("CSP TP %s %s: inverted or zero quote; skipping", ticker, exp_str)
                continue
...
def scan_cc_take_profits(today: dt.date, regime: str = "BULL") -> Dict[str, List[dict]]:
    """Close open CCs that have decayed to the take-profit threshold."""
    ensure_positions_files()
    ...
    for r in rows:
        ...
        try:
            ...
            ask = safe_float(row.get("ask"), 0.0)
            if bid <= 0 or ask <= 0 or ask < bid:
                log.info("CC TP %s %s: bad quote; skipping", ticker, exp_str)
                continue
            ...
            log.warning("scan_cc_take_profits: chain fetch failed for %s %s: %s", ticker, exp_str, e)
            continue
    ...
    if changed:
        ...
        try:
            ...
            if _whl is None:
                raise ImportError("wheel module not found in sys.modules")
            ...
            if lots_changed:
                ...
                log.info("Cleared has_open_cc on %d lot(s) after CC TP close", len(tp_lot_ids))
        except Exception as e:
...
def scan_early_assignments(today: dt.date) -> Dict[str, List[str]]:
    ...
    for r in rows:
        ...
        if current_price > strike * (1.0 - threshold):
            continue
...
def decide_cc_strike(
    ...
    if pct_vs_basis >= 0.0:
        mult = CC_ATR_MULT_NORMAL; tier = "NORMAL"
    elif pct_vs_basis >= -float(CC_UNDERWATER_MILD_PCT):
        mult = CC_ATR_MULT_MILD;   tier = "MILD"
    elif pct_vs_basis >= -float(CC_UNDERWATER_DEEP_PCT):
        mult = CC_ATR_MULT_DEEP;   tier = "DEEP"
    else:
        mult = CC_ATR_MULT_SEVERE; tier = "SEVERE"
...
def plan_covered_calls(today: dt.date, assigned_rows: List[dict], open_cc_lot_ids: set) -> List[dict]:
    ...
    for pos in assigned_rows:
        ...
        if CC_STRIKE_FLOOR_BELOW_CURRENT_PCT > 0:
            ...
            if raw_target < floor:
                log.info("CC %s: raw target %.2f below current-price floor %.2f; skipping",
        ...
        try:
            ...
            if not exp_str:
                log.info("CC %s: no expiry in DTE window (tier=%s %d–%d)", ticker, cc_tier, dte_min, dte_max)
                continue
            chain_key = f"{ticker}-{exp_str}"
            if chain_key not in _chain_cache:
                _chain_cache[chain_key] = t.option_chain(exp_str)
            ...
            if CC_STRIKE_FLOOR_BELOW_CURRENT_PCT > 0:
                floor = current_price * (1.0 - float(CC_STRIKE_FLOOR_BELOW_CURRENT_PCT))
                if strike < floor:
                    log.info("CC %s: rounded strike %.2f below floor %.2f; skipping",
                             ticker, strike, floor)
                    continue
            ...
            net_credit   = fill_credit - (commission / 100.0)
...
def execute_cc_roll(
    ...
    try:
        ...
        if not new_exp or new_exp <= old_exp:
            return FAIL(f"Could not find a later expiry than {old_exp} for {ticker}.")
    except Exception as e:
    ...
    try:
        ...
        if roll_up:
            _dec, raw_target, _reason = decide_cc_strike(current_price, net_basis_per_sh, atr)
        ...
        if CC_STRIKE_FLOOR_BELOW_CURRENT_PCT > 0:
            floor = current_price * (1.0 - float(CC_STRIKE_FLOOR_BELOW_CURRENT_PCT))
            if new_strike < floor:
                return FAIL(f"New strike {new_strike:.2f} below OTM floor {floor:.2f}. Wait.")
    except Exception as e:
    ...
    try:
        ...
        if bid_new < float(CC_MIN_BID):
            return FAIL(f"New CC bid {bid_new:.2f} below minimum {CC_MIN_BID}.")
    ...
    if net_credit < 0:
        return FAIL(
            f"Roll is a net DEBIT of ${abs(net_credit):.2f}. Debit rolls blocked. "
            f"Wait for more decay or try roll-out only."
        )
    ...
    try:
        ...
        new_idea = {
            ...
            "reason": f"[ROLL {'UP+OUT' if roll_up else 'OUT'}] {cc_tier} → {new_exp} {new_strike:.0f}C | net ${net_credit:.0f}",
        ...
        log.info("CC rolled %s: %s %.0fC → %s %.0fC | buyback $%.0f credit $%.0f net $%.0f",
                 ticker, old_exp, old_strike, new_exp, new_strike, buyback_total, credit_total, net_credit)
        ...
        return {
            ...
            "reason": (
                ...
                f"buyback ${buyback_total:.0f}  new credit ${credit_total:.0f}  net credit ${net_credit:.0f}"
            ),
...
def execute_cc_close_and_exit(
    ...
    try:
        ...
        log.warning("execute_cc_close_and_exit: CC quote failed for %s, treating as $0: %s", ticker, e)
    ...
    lots       = _whl._read_rows(_whl.WHEEL_LOTS_FILE)
    parent_lot = next(
        (l for l in lots if (l.get("lot_id") or "").strip() == source_lot_id), None
    ) if source_lot_id else None

    if not parent_lot:
        return FAIL(f"Could not find wheel lot {source_lot_id!r} for {ticker}.")
    ...
    try:
        ...
        _whl._append_stock_trade_record({
            ...
            "entry_date":  open_date, "entry_price": f"{entry_per_sh:.4f}",
        ...
        _whl.record_event(
            date=today.isoformat(), account=account, ticker=ticker,
        ...
        log.info("CC manual exit — %s %d sh sold @ $%.2f, CC buyback $%.0f, P&L $%.0f (%.1f%%)",
                 ticker, shares, exit_price, cc_buyback_total, net_pnl, net_pnl_pct)
